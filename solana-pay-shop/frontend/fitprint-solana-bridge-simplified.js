// FitPrint Solana Pay Integration Bridge - Simplified Categorized Version
class FitPrintSolanaPayBridge {
    ...
    init() {
        console.log('🔗 Initializing FitPrint Solana Pay Bridge (Categorized)...');
    ...
    setupMessageListener() {
        // Listen for messages from the FitPrint iframes
    ...
    checkForFitPrintUpdates() {
        // This would normally check iframes for cart updates
    ...
    initializeCategorizedIframes() {
        console.log('🎨 Initializing categorized iframes...');
        
        // List of all category iframes
        ...
        // Initialize each iframe when it loads
        categoryIframes.forEach(iframeId => {
            const iframe = document.getElementById(iframeId);
            if (iframe) {
                console.log(`🔧 Setting up ${iframeId}...`);
                this.setupCategoryIframeEvents(iframe, iframeId);
            }
        });
        
        // Add scroll spy for active category highlighting
        this.setupCategoryScrollSpy();
    }
    
    setupCategoryIframeEvents(iframe, iframeId) {
        const category = iframeId.replace('Iframe', '');
        
        // Add loading state
        const container = iframe.closest('.category-iframe-container');
        if (container) {
    ...
    setupCategoryScrollSpy() {
        console.log('👀 Setting up category scroll spy...');
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    // Could add active category highlighting here
                }
            });
        }, {
            threshold: 0.3
    ...
    extractFitPrintCartData() {
        // Extract cart data from FitPrint iframes if possible
